...
This chart install the Floki application to ship logs to Loki. It defines daemonset on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
The command deploys Floki on the Kubernetes cluster with the default configuration. The [configuration](#configuration) section lists the parameters that can be configured during installation.
...
Alternatively you can install the full [Loki stack](../loki-stack) (Loki + Floki) using:
...
By default, `rbac.create` is set to true. This enable RBAC support in Floki and must be true if RBAC is enabled in your cluster.

The chart will take care of creating the required service accounts and roles for Floki.
...
The following tables lists the configurable parameters of the Floki chart and their default values.

For more details, read the [Floki documentation](../../../cmd/fluent-bit/README.md)
...
| `config.port`            | the Floki port to listen. (This is mainly used to serve metrics)                              | `2020`                           |
| `config.loglevel`        | the Floki log level (debug,info,warn,error).                                                  | `warn`                           |
| `config.lineFormat`      | The line format to use to send a record (json/key_value)                                           | `json`                           |
| `config.k8sLoggingParser`| Allow Kubernetes Pods to suggest a pre-defined Parser. See [Official Floki documentation](https://docs.fluentbit.io/manual/filter/kubernetes#kubernetes-annotations).                                                                                      | `Off`                           |
...
| `config.labelMap`        | Mapping of labels from a record. See [Floki documentation](../../../cmd/fluent-bit/README.md) |                                  |
| `config.parsers`         | Definition of extras Floki parsers. See [Official Floki documentation](https://docs.fluentbit.io/manual/filter/parser). The format is a sequence of mappings where each key is the same as the one in the [PARSER] section of parsers.conf file       | `[]`                            |
...
| `image.repository`       | The Floki docker image repository                                                             | `grafana/fluent-bit-plugin-loki` |
| `image.tag`              | The Floki docker image tag                                                                    | `0.1`                            |
| `image.pullPolicy`       | The Floki docker image pull policy                                                            | `IfNotPresent`                   |
| `nodeSelector`           | Floki [node labels][nodeSelector] for pod assignment                                          | `{}`                             |
| `podLabels`              | additional Floki pod labels                                                                   | `{}`                             |
| `podAnnotations`         | additional Floki pod annotations                                                              | `Prometheus discovery`           |
...
| `serviceMonitor.enabled` | Create a [Prometheus Operator](operator) serviceMonitor resource for Floki                    | `false`                          |
